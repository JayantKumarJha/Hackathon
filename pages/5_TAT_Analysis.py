...
# ----------------------
# TAT (Turnaround Time) Breach Analysis — Page 6 for Streamlit app
...
def load_and_prepare(uploaded_file):
    ...
    # parse dates (day-first parsing — India)
...
if not st.session_state.get("tat_data_file_loaded"):
    uploaded_file = st.file_uploader("📤 Upload Excel for TAT analysis (single sheet expected)", type=["xlsx", "xls"])
else:
    uploaded_file = None
...
st.sidebar.header("🎛 MatType filter & date range")
...
st.sidebar.write(f"Data range (Testing Slip Date): {min_test_dt} → {max_test_dt}")

# Date range selector (calendar) — default to full available range
...
if not breaches_df.empty:
    ...
    st.subheader("🚨 TAT Breached Items (preview)")
...
# Plot weekly breach %
st.subheader("📈 Weekly TAT Breach (%) — cohort = Testing Slip week")
if weekly.empty:
...
st.subheader("⬇️ Download breached items")
if not breaches_df.empty:
    ...
    # convert to Excel-friendly (keep as datetimes—pandas will write them)
    # Build the bytes
    try:
...
# ----------------- Debug / info -----------------
with st.expander("🔎 Debug / Info (useful)"):
    st.write("Selected MatTypes:", selected_mattypes)
    st.write(f"Testing Slip Date range available: {min_test_dt} → {max_test_dt}")
